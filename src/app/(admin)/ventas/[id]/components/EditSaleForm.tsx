...
export default function EditSaleForm({ venta, clientes, productos }: Props) {
  const [customerId, setCustomerId] = useState(venta.customerId);

  const [selectedCustomer, setSelectedCustomer] = useState(venta.customer);
  ...
  const [discount, setDiscount] = useState(String(venta.discount || 0));
  ...
  return (
    <form
      action={async (formData) => {
        try {
          await updateSale(venta.id, formData);
          ...
          toast.error(
            error instanceof Error ? error.message : "Error al editar venta",
          );
        }
      }}
    ...
    >
      ...
      <input type="hidden" name="invoice" value={venta.invoice || ""} />
